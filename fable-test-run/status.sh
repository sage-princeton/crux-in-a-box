...
echo "====== CRUX fable smoke test  ·  Linux status ======"
echo "  (no external reviews · no GitHub · no Google)"
echo ""
...
# ----- git (local only; no GitHub remote in this variant) -----
if command -v git &>/dev/null; then
  if [ -n "$(git config --global user.email 2>/dev/null)" ]; then
    pass "git: installed, commit identity set (local-only, no remote)"
  else
    warn "git: installed but no global user.email — commits will fail"
  fi
else
