...
# ==========================================================================
# setup-device.sh — SMOKE-TEST VARIANT (fable-test-run)
# ==========================================================================
...
#
# This is the smoke-test fork of linux/setup-device.sh. Three external
# dependencies are removed wholesale so a run can be launched with nothing but
# an Anthropic key, a Telegram bot, and (optionally) RunPod:
#   - NO external reviewers  (no refine.ink / CMU / Stanford; no REFINE_INK_API_KEY)
#   - NO GitHub              (no gh CLI, no PAT, no remote; git stays local)
#   - NO Google              (no gog / gogcli, no Gmail bundle, no GOG_* keys)
# Everything else — the workspace, the gates, the critics — is the full harness.
# See README.md in this directory for what that changes about the run.
#
...
#   4. Copies this fable-test-run/ directory (scripts + workspace) to the box.
#   5. Runs the remote bootstrap (src/start.sh) which installs a desktop
#      environment, VNC server, openclaw, monitoring, telemetry, etc.
...
absent or blank):
  ...
  Workspace placeholders (substituted into the harness files):
    ...
    RESEARCH_QUESTION    The one-sentence research question
    RESEARCH_CONTEXT     Background + exactly what to produce (one line)

Not used by this variant (present in linux/setup-device.sh, removed here):
    REFINE_INK_API_KEY, GITHUB_USER, GITHUB_CLASSIC_PERSONAL_ACCESS_TOKEN,
    GOG_ACCOUNT, GOG_KEYRING_PASSWORD, GOG_HOME_TARBALL. If any of these are
    present in a config file carried over from linux/, they are ignored — they
    are on the NON_PLACEHOLDER_KEYS list, so they are never sed'd into the
    workspace and never forwarded to the box.
...
Optional (override via env vars):
  ...
  CRUX_INSTANCE_NAME             Instance Name tag (default: crux-fable-test)
...
# keeps CLOUD_SPEND_LIMIT, API_BUDGET and any optional placeholder
# (PAGE_BUDGET, VENUE, …) flowing into the harness files.
#
# The three removed integrations' keys stay on this ignore list so a config
# file carried over from linux/placeholders.txt doesn't sed dead credentials
# into the workspace.
NON_PLACEHOLDER_KEYS=" TELEGRAM_BOT_NAME TELEGRAM_OWNER_ID ANTHROPIC_MODEL ANTHROPIC_API_KEY REASONING_EFFORT COST_TRACKER_URL RUNPOD_API_KEY INSTANCE_SUFFIX REFINE_INK_API_KEY GITHUB_USER GITHUB_CLASSIC_PERSONAL_ACCESS_TOKEN GOG_ACCOUNT GOG_KEYRING_PASSWORD GOG_HOME_TARBALL "
PLACEHOLDERS=""
for key in "${!CFG[@]}"; do
...
INSTANCE_NAME="${CRUX_INSTANCE_NAME:-crux-fable-test}"
...
# ====== 7. COPY FILES ======
# Unlike linux/setup-device.sh, this variant is self-contained: the provisioning
# scripts and the harness workspace live in the same directory, so one copy does
# both. start.sh reads the workspace from ~/crux-in-a-box-linux/workspace.
info "Copying fable-test-run/ directory to instance..."
scp -o StrictHostKeyChecking=no -i "$KEY_FILE" -r \
  "$SCRIPT_DIR" "${SSH_USER}@${PUBLIC_IP}:~/crux-in-a-box-linux"
ok "Provisioning scripts + harness workspace copied"

[ -d "$SCRIPT_DIR/workspace" ] \
  || warn "workspace/ not found in $SCRIPT_DIR — workspace setup will be skipped on the box"
...
============================================
  CRUX fable smoke-test instance is ready!
  (no external reviews · no GitHub · no Google)
============================================
...
cat <<'EOF'
  ...
  ── Retrieving the run (no GitHub remote) ────
  The agent commits locally only. Pull the repo
  and the paper down yourself when it finishes:

    scp -i <key> -r ubuntu@<ip>:~/.openclaw/workspace ./run-output
  ─────────────────────────────────────────────
EOF
