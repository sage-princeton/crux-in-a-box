...
#              SMOKE-TEST VARIANT (fable-test-run)
# ==========================================================================
# Installs a desktop environment, VNC, openclaw, monitoring, and telemetry.
#
# Differs from linux/src/start.sh by omission — three integrations are gone:
#   - GitHub:  no gh CLI install, no PAT auth, no GITHUB_TOKEN/GH_TOKEN.
#              git is installed and works locally; there is no remote.
#   - Google:  no gogcli install, no gog auth bundle, no GOG_* env.
#   - External review: no REFINE_INK_API_KEY in ~/.openclaw/.env.
# The harness workspace is read from ~/crux-in-a-box-linux/workspace (this
# variant ships scripts and workspace in one directory).
#
...
# (used by the gateway's model calls), this one is consumed by the agent's bash
# tool calls — RunPod GPU pods — so it must reach the tool environment (the
# gateway loads ~/.openclaw/.env into its process env, which tool subprocesses
# inherit). REFINE_INK_API_KEY is deliberately absent: this variant runs no
# external reviews.
...
# ====== SERVICES ======
#
# Deliberately NOT installed in this variant (see the header):
#   - GitHub CLI (gh)      — no GitHub in the smoke test; git stays local.
#   - gogcli (gog)         — no Google Workspace / Gmail.
# The corresponding auth blocks are gone too. Do not re-add one without also
# re-adding its harness-side references (TOOLS.md, review.md, gate_artifact.sh).
...
# ====== LOCAL GIT IDENTITY ======
# There is no GitHub remote in this variant, but the agent still commits
# constantly (AGENTS.md § Git discipline). Give git an identity so commits
# don't fail on a fresh box, and default the branch name.
...
echo "✔ Local git identity configured (no remote — commits stay on the box)"

# ====== HARNESS WORKSPACE ======
# Copy the harness workspace into the agent's OpenClaw workspace and resolve
# placeholders that are known at provisioning time. This variant ships the
# workspace alongside the provisioning scripts, so it comes from the same
# copied directory rather than a separate ~/crux-in-a-box-harness.
HARNESS_SRC="$REAL_HOME/crux-in-a-box-linux/workspace"
