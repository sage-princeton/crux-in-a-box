...
while true; do
  ...
  # Use scrot for screenshots on Linux (equivalent of macOS screencapture -x)
  DISPLAY=:1 scrot "$TEMP_SCREEN" 2>/dev/null || true

  if [ -f "$TEMP_SCREEN" ]; then
  ...
  if [ -d "$TARGET_DIR" ]; then
    ...
    # Uses Linux stat format (vs macOS stat -f)
    CURR_DIR_HASH=$(find "$TARGET_DIR" -type f -exec stat --format="%Y %s %n" {} + 2>/dev/null | md5sum | awk '{print $1}')
