...
It guards **ship only**. (Smoke-test variant: two ship gates, not three — see below.) Drafting is gated cooperatively by the isolated exploration-sufficiency critic's `reviews/*.md` verdict (playbooks/exploration.md), not by a flag this plugin could enforce. The ship gates it runs:

- ~~`REQUIRE_EXTERNAL_REVIEWS`~~ — **not used in this run.** There are no external reviewers (`playbooks/review.md` §2b), and `gate_artifact.sh` fails if the flag is set, so it is omitted from `SHIP_FLAGS`.
...
(denylist beats allowlist). Provide `scripts/ship.sh` that runs `REQUIRE_EVIDENCE_ADEQUATE=1 REQUIRE_SHIP_AUTHORIZATION=1 REQUIRE_PRESENTATION=1 REQUIRE_README=1 scripts/gate_artifact.sh <pdf>` and only proceeds on exit 0.
