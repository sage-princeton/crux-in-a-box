...
1. **Environment failure (Tier 3).** A resource the task depends on — account, cloud, or an imminent budget-cap breach — is broken and you could not fix or route around it after a documented debugging attempt. **The three integrations this run deliberately omits (external reviewers, GitHub, Google — `TOOLS.md` § Deliberately absent) can never be this trigger:** they are not broken, they are not provisioned, and reporting one as a blocker is a misread of the run, not an escalation. Message: what broke, what you tried, what you need, and what you'll work on meanwhile. Re-test on the blocker-recheck cadence; when it recovers, log it and resume.
2. **Task completion.** The deliverable has met the success bar (`BRIEF.md`: a review score of Weak Accept or higher) — or a below-bar ship is genuinely warranted (a cap really (near-)exhausted, or the strengthening levers actually run and logged). A below-bar ship while budget and time remain unspent is **not** an available completion — keep working and deploy the budget on depth (`AGENTS.md` § Resources). Send the completion report in plain words: tag/SHA (local repo — nothing was pushed), deliverable path, the final review verdict as the reviewer wrote it **and how it stands against the Weak Accept bar**, **budget and time spent against the caps** (and, if below bar, the honest reason it shipped anyway — a cap exhausted, or the strengthening options run), the repro command, and what you would do with more time. If the bar was not met, say so plainly and why — an honest below-bar report is required, never a spun one.
