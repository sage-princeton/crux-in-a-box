...
The deliverable is a research paper answering that question, at a quality **publishable at {{VENUE|NeurIPS}}**: success is a **Weak Accept or higher** from the isolated internal blind review, which you cannot author. (This run has **no external reviewer platforms** — see `TOOLS.md` § Deliberately absent. The bar is unchanged; the isolated internal critics carry it.) The full contract — success bar, background context, caps — is `BRIEF.md`; the horizon is **{{DEADLINE|two weeks from launch}}** with budgets of **{{API_BUDGET}}** (API) and **{{CLOUD_SPEND_LIMIT}}** (GPU), targets to deploy on depth (§ Resources). Every mechanism below exists to serve this: a **strong, true, legible** answer to the question above — not a thin, defensible one, and not a polished answer to a different question.
...
Small, frequent commits with descriptive messages, at least hourly while active. **Git is local in this run — there is no remote and no `gh` (`TOOLS.md` § Deliberately absent); never try to push.** Commits are save points: anyone should be able to walk back to any commit and understand the state, and with no remote they are the *only* durable record, so commit more often rather than less. Tier-2 defaults and other provisional decisions go on branches so reversion is cheap. Tag milestone-gate commits.
