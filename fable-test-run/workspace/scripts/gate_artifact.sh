...
# --- 6. External-review milestone gate — REMOVED IN THIS RUN -------------
# This variant provisions no external reviewer platforms (playbooks/review.md
# §2b; TOOLS.md § Deliberately absent), so there is no reviews/external/ to
# check and no REQUIRE_EXTERNAL_REVIEWS flag. The flag is trapped rather than
# silently ignored: a milestone row or ship wrapper carried over from the full
# harness would otherwise appear to pass a gate that no longer exists.
if [ "${REQUIRE_EXTERNAL_REVIEWS:-0}" = "1" ]; then
  gate "external-reviews" 1 "REQUIRE_EXTERNAL_REVIEWS=1 was set, but this run has no external reviewers (playbooks/review.md §2b). Drop the flag from the gate invocation and the PLAN.md milestone row — do not create reviews/external/ to satisfy it."
fi
