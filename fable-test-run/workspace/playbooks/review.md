...
> **This run has no external reviewers.** The §2b external-review milestone is removed (`TOOLS.md` § Deliberately absent) — no CMU/Stanford portals, no refine.ink, no `reviews/external/`, no `REQUIRE_EXTERNAL_REVIEWS` gate. Every reviewer here is an **isolated internal critic**: §2a blind reviewers, §2c the accompanying final review, §2d the power critic, §3 brief-fidelity. That makes their isolation the *only* thing standing between you and a self-graded paper — the spawner-side contamination ban and the PDF-only input rule are load-bearing, not ceremony. §2b's absence removes corroboration, not the Weak Accept bar.
...
**Success bar (`BRIEF.md`):** the run succeeds when the blind-review recommendation reaches **Weak Accept or higher** (internal §2a / §2c). Convergence *below* the bar (see the stopping rule) means *reviewing* is exhausted, not that the bar is met — a converged sub-Weak-Accept verdict is failure-to-meet-bar: it routes to the stuck/pivot fork (`playbooks/decisions.md`) — default, return to the Experiment phase to close the gap — while budget and time remain, and is shipped only when the budget or deadline is exhausted (honestly, with the gap stated in the completion report, `USER.md`). Never manufacture the score — reviewers are unauthorable, so the only path to the bar is genuinely meeting it.
...
3. _(§2b external reviews — removed in this run.)_
...
6. **Final README** (`playbooks/writing.md` § The final README) — the cold-visitor front door for the repo, written after the overhaul passes, gated (`REQUIRE_README=1`) plus one PDF-free cold-reader on the README alone, committed as the run's last commit (local only — there is no remote to push to).
...
- Stop when a round adds **no new soundness finding** — i.e. when the verdict and the set of soundness/claim-level weaknesses are unchanged from the prior round (**identical verdict two rounds running = done**). At that point further internal rounds carry zero information; proceed to the accompanying final round (§2c). (Presentation-level findings get batched and fixed but never keep the loop alive.)
- Keep a **sane upper bound** as a sanity heuristic, not a counter to game: blind-review verdicts plateau within ~5 rounds, so if you reach roughly half a dozen rounds without convergence, treat the *process* as the problem (contaminated reviewers, a moving artifact) and proceed to §2c rather than spawning more. There is no streak to track in any file.
- **With no §2b to follow, the stop is load-bearing in a way it usually isn't.** Convergence here goes straight to the final review and the ship gates — there is no outside slate coming to catch what the internal rounds missed. So resist both failure directions equally: don't stretch the loop past its yield hoping for absolution, and don't call round two "converged" because nothing external is waiting. If a round's findings are *unchanged but weak*, the honest move is usually a closing experiment, not another reviewer.
...
### §2b — External reviews — REMOVED IN THIS RUN

There are no external reviewer platforms on this box: no CMU portal, no Stanford portal, no refine.ink key, no Gmail to receive a review, no `reviews/external/` directory, and no `REQUIRE_EXTERNAL_REVIEWS` gate in `scripts/gate_artifact.sh` (`TOOLS.md` § Deliberately absent). This is a deliberate configuration of the run, **not a broken resource** — it is not a Tier-3 block, and finding a substitute review service is out of scope. Do not spend turns on it.

Two consequences to actually carry:

1. **Don't recreate the milestone.** No `PLAN.md` row should gate on an external artifact, and no gate invocation should set `REQUIRE_EXTERNAL_REVIEWS=1`. The final gate for this run is in `PLAN.md` § Milestone table.
2. **Don't quietly promote yourself into the vacancy.** The reason §2b existed is that internal rounds can drift toward the author's own frame. With it gone, the guard is the isolation discipline on the remaining critics — a fresh subagent per round, PDF-only inputs, and a spawn block carrying no round number, no prior verdict, and no expected outcome. An "external-style review" you write, or a reviewer you brief with context, is worse than no review at all: it manufactures the score. If you want a genuinely independent read, the honest form is another **isolated** §2a round under the same rules.

**Synthesizing the slate — the over-index heuristic.** Still applies across the internal rounds (§2a rounds plus the §2c accompanying final review). Do not let one favorable verdict outweigh the rest. The judgment to apply (you apply it; nothing in a file mechanizes it): **a lone at/above-bar review cannot overrule a soundness objection that ≥2 other reviews raise on the *same axis*.** If round A calls a design choice sound and rounds B and C flag the *same* choice as a soundness problem, the slate is **below-bar on that axis**, not "mixed" — one positive vote does not rule the contested point settled. A genuinely **MIXED** slate requires the positive and negative verdicts to rest on *different* axes (e.g. accept on method, reject on scope), not one positive overruling same-axis negatives. When the majority — including the §2c accompanying final review — names the same central defect, that is **convergence below bar**: route to Stuck/Pivot (default, return to Experiment to close the defect); record the synthesis honestly in the completion report (do not headline the lone accept). The bar is met only when the *unauthorable* reviewers actually reached Weak Accept+, not when one did while others dissented on soundness.

### §2c — The accompanying final review

After the §2a rounds converge, one last internal blind round under §2a. Its full output ships with the deliverable as `reviews/final_review.md`, verdict **as the reviewer wrote it** — if it says Weak Reject, it ships saying Weak Reject. Add a ≤6-line author annotation at top: what was fixed in response to which round, and what you chose not to fix and why.
