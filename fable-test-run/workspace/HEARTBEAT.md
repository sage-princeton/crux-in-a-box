...
tasks:
  ...
  - name: git-hygiene
    interval: 60m
    prompt: "git status — if the tree is dirty, commit with a descriptive message. Git is LOCAL in this run (no remote, no gh — TOOLS.md § Deliberately absent): do not attempt a push, and do not log a failed push as a blocker. Commits are the only durable record of the run, so err toward committing."
