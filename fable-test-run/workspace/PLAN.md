...
- _The **final README** is the run's last commit: after the Presentation Overhaul and before the completion report, write the repo-root README for a visitor with zero context (`playbooks/writing.md` § The final README), pass its gate + one cold-reader, commit (locally — there is no remote in this run), then send the completion report._
...
| 4 | Review: hostile + internal blind rounds converge (`playbooks/review.md`; no external reviews in this run); back-edges to Experiment expected | | `scripts/gate_artifact.sh <pdf>` + §2a yield-based stop reached | |
