...
Why this exists: the dominant front-of-run failure is racing to a deliverable — a one-pass literature skim and an hours-long candidate portfolio, then committing to the first idea that survives a single shallow scout. Exploration is the phase the rest of the scaffold is built to *race past*. The agent should spend a substantial front fraction of the run here — as a calibration, roughly 30–40% of the horizon *whatever its length* (on a two-week run, the first four or five days; on a 1-day run, the first seven to nine hours; a short deadline compresses drafting and review, never exploration) — across **both literature and experimentation**, before any prose — but exploration is a *cycle*, not an hour-0 gate: `Set up → Explore ⇄ Experiment ⇄ Draft ⇄ Review` (`PLAN.md` § Research plan). Literature and scouting recur against open cruxes for as long as cruxes stay open. This playbook is the authority `PLAN.md` and `AGENTS.md` point at for what "explore deeply" means, and the home of the isolated exploration-sufficiency critic that authorizes drafting.

**What scales with the horizon is volume, not structure.** On a short run the dossier gets *fewer* anchor papers (3 rather than 5), cheaper scouts, and tighter subagent budgets — but every required section below still exists, every anchor named still gets a deep-read note, and the §4 sufficiency critic still gates drafting. Shrinking the fraction of the run spent exploring, dropping a section, or skipping the critic because "there isn't time" is the exact failure this playbook exists to prevent; a 1-day run with a thin dossier fails faster, not better.
...
**LaTeX source for the best published anchors.** For the **three best published** anchor papers — the closest prior work that is published and available on arXiv — also fetch the actual **LaTeX source** (arXiv serves it at `https://arxiv.org/e-print/<arxiv-id>`; the tarball holds the `.tex` and figures) into `paper/exemplars/<paper>/`. It earns its place twice: it lets you read **deeper than the rendered PDF** when a crux turns on a method's exact derivation, setup, or notation; and these `.tex` files become the **structural templates** for drafting the paper (`playbooks/writing.md` § Structure from exemplars). This is a *resource, not a boundary* — it does **not** narrow the review to these three: the survey, the full anchor deep-read, and the recurring crux-driven reading all stay broader.
