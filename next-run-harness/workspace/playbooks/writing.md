...
3. **Bullet outline of the full paper** — every section has a named job for a skeptical reader. Compare the section plan against 1–2 well-structured exemplars from the lit synthesis (or the target venue) — match how strong papers in the area sequence the argument.
...
- **Structure from exemplars (informs the edit, not the cold-read).** Pull 2–3 well-structured, accepted papers in the area — from `exploration/DOSSIER.md` § LIT SYNTHESIS or the target venue, chosen for being *well-written*, not just closest prior work. If they aren't already on disk in `exploration/lit/`, a subagent fetches them into `paper/exemplars/` so they're in context. Use their front-matter architecture as a template — how they open the abstract, sequence the introduction's arc, phrase the contributions list, and frame Figure 1 — matching the *shape*, never the content.
- **One focused edit** in response to the cold-read and guided by those exemplars: tighten the abstract to the sentence-roles above; make the introduction's contribution and contributions-list legible on a single read; fix Figure 1's takeaway and caption (`playbooks/figures.md`). Keep it front-matter-scoped — if the comparison surfaces a *deep* structural mismatch (organization, not polish), log it as a finding for triage (`playbooks/decisions.md`); do not silently restructure a blind-reviewed artifact at camera-ready.
