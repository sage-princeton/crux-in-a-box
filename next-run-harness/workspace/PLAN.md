...
_One paragraph: what the brief asks for, restated in your own words. (Write in the first ~5% of the run horizon — `BRIEF.md` § Budgets.)_

## Research plan (the spine)

_The intellectual center of the run — a **living, two-pass** artifact, not an hour-0 commitment. **Seed it in the first ~5% of the horizon** with only what is honest before reading the literature: the question restated, and provisional hypotheses/directions to *aim* the lit review (the budget capacity is costed in § Budget-deployment menu). The lit-dependent parts below — the calibrated *strong-vs-weak* target, the single most decisive experiment, the real cruxes, and the committed direction — are the **output of the Exploration Dossier** (`playbooks/exploration.md`) and get filled in / revised there, gated by the exploration-sufficiency critic; writing them as hour-0 guesses is the premature-commitment trap exploration exists to avoid. Revisit the spine at every milestone. This is the north star for **quality**, not just defensibility. Five parts:_
...
_Write within the first ~5% of the run horizon (`BRIEF.md` § Budgets); log as the first Tier-2 memo. The lifecycle is a **cycle, not a staircase**:_

`Set up → Explore (deep-read lit + scouts) ⇄ Experiment ⇄ Draft ⇄ Review → Presentation Overhaul → Ship`
...
- _3–6 milestones, each with an **absolute** deadline (a timestamp, computed from `locks/budget.json` `launch_iso`/`deadline_iso` — never a relative "in 3 days") and a gate command that must pass; every gate includes the brief-fidelity review (`playbooks/review.md` §3)._
- _**Schedule in fractions of the horizon, then convert to timestamps.** The horizon is `{{DEADLINE|1 day from launch}}` (`BRIEF.md` § Budgets) and may be anything from a day to a fortnight; the *proportions* are what carry over. Working split: setup + skeleton by ~10%, Exploration Dossier by ~35–40%, Experiment ⇄ Draft to ~75%, Review to ~90%, Presentation Overhaul + Ship in the last ~10%. On a 1-day run that is: skeleton by hour 2, dossier by hour 9, review from hour 18, overhaul in the last two hours. A short horizon compresses drafting and review, **not** exploration — a thin dossier poisons everything downstream (`playbooks/exploration.md`)._
...
- _The **final gate** runs `REQUIRE_EVIDENCE_ADEQUATE=1 REQUIRE_SHIP_AUTHORIZATION=1 REQUIRE_PRESENTATION=1 REQUIRE_README=1 scripts/gate_artifact.sh <pdf>` — the isolated power critic's `reviews/power_critic_ship.md` ADEQUATE against the delivered artifacts, the light under-spend backstop satisfied, the presentation gates still passing at ship, and the cold-visitor README in place._
...
| 4 | Review: hostile + blind rounds converge under the yield-based stopping rule (`playbooks/review.md` §1/§2a) and the accompanying final review is written; back-edges to Experiment expected | | `scripts/gate_artifact.sh <pdf>` + `reviews/final_review.md` present | |
...
| 6 | Ship: camera-ready + accompanying final review + power critic ADEQUATE + presentation gates hold + final README written, cold-read, and committed as the last commit (`playbooks/writing.md` § The final README) | | `REQUIRE_EVIDENCE_ADEQUATE=1 REQUIRE_SHIP_AUTHORIZATION=1 REQUIRE_PRESENTATION=1 REQUIRE_README=1 scripts/gate_artifact.sh <pdf>` | |
