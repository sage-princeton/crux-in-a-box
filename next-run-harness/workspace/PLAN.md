...
- _Calibrate the schedule to the horizon — **all milestone timing is a fraction of it, never an absolute default** (`AGENTS.md` § Horizon & cadence: Milestone 1 by ~7%, dossier at 30–40%, Presentation Overhaul + Ship in the last ~15%). Compute the horizon in hours at hour-0 and write absolute deadlines into the table below from those fractions. A short horizon compresses drafting and review, not exploration — a thin dossier poisons everything downstream (`playbooks/exploration.md`)._
