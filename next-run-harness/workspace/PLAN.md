...
- _The **final gate** runs `REQUIRE_EXTERNAL_REVIEWS=1 REQUIRE_EVIDENCE_ADEQUATE=1 REQUIRE_SHIP_AUTHORIZATION=1 scripts/gate_artifact.sh <pdf>` — externals present, the isolated power critic's `reviews/power_critic_ship.md` ADEQUATE against the delivered artifacts, and the light under-spend backstop satisfied._
- _**Readability** is a distinct terminal milestone after Review converges and before Ship — a full-paper readability round, not a front-matter pass (`playbooks/writing.md`). It is a craft pass: it does not reopen claims or re-run experiments._
...
| 5 | Readability: full-paper cold-read round + one focused clarity edit (`playbooks/writing.md`) | | `scripts/gate_artifact.sh <pdf>` (form gates) | |
| 6 | Ship: externals reviewed + camera-ready + accompanying final review + power critic ADEQUATE | | `REQUIRE_EXTERNAL_REVIEWS=1 REQUIRE_EVIDENCE_ADEQUATE=1 REQUIRE_SHIP_AUTHORIZATION=1 scripts/gate_artifact.sh <pdf>` | |
