...
- **Version control:** `git`, **local only — there is no remote and no `gh`.** Commit often (`AGENTS.md` § Git discipline); never plan a `git push`, and don't spend turns trying to create a remote. Consequence to design around: **the box is the only copy of your work.** Nothing is backed up off-host, so treat `locks/`, `LOG.md`, `REGISTRY.md`, and the paper source as unrecoverable if lost — commit early and often so at least the local history survives a bad edit.
- **Email:** none. There is no mail client and no inbox on this host — any procedure that depends on receiving email is unavailable to you (this is why the external-reviewer slate is API-only, below).
...
- **External reviewer (`playbooks/review.md` §2b — submit the camera-ready PDF, save the returned review into `reviews/external/`):**
  - **refine.ink** — programmatic REST API; key in env `REFINE_INK_API_KEY`. Submit the PDF and fetch the review via the API — **read refine.ink's API docs hour-0 for the exact endpoints/shape and verify the key works before relying on it.** If it is metered/paid, run it LAST (the §2b ordering: paid single-shot reviews go after internal rounds are clean).

  **This is the only external reviewer available, and that raises the stakes on it.** Email-delivered portal reviewers (CMU, Stanford) are *not* available on this host — no inbox exists (see § Accounts → Email), so do not attempt them and do not treat their absence as a defect to route around. Two consequences: (1) verify the refine.ink key hour-0, because a single broken reviewer is now a Tier-3 block with no fallback (`USER.md`); (2) the unauthorable-verdict requirement rests on this one external review plus the isolated internal blind reviews (`playbooks/review.md` §2a/§2c) — weight the internal slate accordingly rather than treating one API verdict as decisive.
