...
openclaw agent --session-key agent:main:main --timeout 900 --message "[Environment watchdog — automated recovery, not an operator instruction] The previous main session hit an unrecoverable provider error (cascading invalid-thinking-signature, a known OpenClaw bug) and was automatically reset; the old transcript is archived in the session store. Recover per AGENTS.md § Session startup: read the state capsule at the top of today's memory file, the tail of LOG.md, and the PLAN.md work queue; check running jobs (ps, PID files) and in-flight subagents; re-arm any harvest crons or self-chains that were lost with the session; add a one-line LOG.md note that this reset happened; then take the next forward action per the End-of-Turn Contract." >> "$LOG" 2>&1
