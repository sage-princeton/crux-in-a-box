...
#   5. Runs the remote bootstrap (start-claude.sh) which installs a desktop
...
# env-forwarding to start-claude.sh), so they get their own variables.
TELEGRAM_BOT_NAME="${CFG[TELEGRAM_BOT_NAME]}"
TELEGRAM_OWNER_ID="${CFG[TELEGRAM_OWNER_ID]}"
ANTHROPIC_MODEL="${CFG[ANTHROPIC_MODEL]}"
ANTHROPIC_API_KEY="${CFG[ANTHROPIC_API_KEY]}"
# Extended-thinking level — enables extended thinking. Default "xhigh"; start-claude.sh
...
if [ -n "$INSTANCE_SUFFIX" ]; then
  ...
  INSTANCE_NAME="${INSTANCE_NAME}-claude"
fi
