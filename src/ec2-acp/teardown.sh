...
# crux-run-sg, the key pair or the IAM role: those are shared with the control
# box and other run boxes, and deleting them would break the next launch.
#
...
echo "  delete SSM parameter $SSM_ENV_PARAM"
echo "  remove ~/.ssh/config entry for $SLUG"
echo
echo "Keeping (shared): crux-run-sg, the key pair, crux-run-role, and the control box."
echo
...
# The parameter holds the OpenAI key and the workspace token, so it goes with
# the box rather than lingering after it.
if aws_ ssm get-parameter --name "$SSM_ENV_PARAM" >/dev/null 2>&1; then
  aws_ ssm delete-parameter --name "$SSM_ENV_PARAM" >/dev/null
  ok "Deleted $SSM_ENV_PARAM"
else
  warn "$SSM_ENV_PARAM not present"
fi
