...
**Two LLM budgets, never crossed.** Your own turns, heartbeats, and subagents bill to the API budget the harness meters; your experiments' model calls bill to the provisioned experiment-LLM provider's key (§ Environment), and only there. Experiment code never uses the harness's model path (the provider variables in your environment point at the metered bridge, and SDKs pick them up silently), and a subagent is never the experimental model — either one spends the wrong budget and makes both ledgers wrong.
...
- **Nothing you start is guaranteed to outlive your turn.** Your subagents are threads of your own session process and end the moment your turn ends; background processes and persistent tool sessions started from a turn have died with it too. Ending a turn with work in flight therefore kills it silently: on the next heartbeat there is no report, no file, no verdict, and nothing to harvest — it looks like a broken resource when it is your own yield. So never end a turn with delegated or background work in flight: wait on it (`wait_agent` for subagents, looping if it needs longer than one wait; poll a job's output for processes), harvest, then yield. The one launch form verified to survive a turn boundary here is a detached session: `setsid nohup <cmd> > runs/<name>/out.log 2>&1 < /dev/null &` (a plain `nohup … &` child dies with the turn). Use it for every background job, record the PID, and still design long jobs to checkpoint to disk, so that a job lost for any reason costs minutes, not hours.
...
- **Workspace:** `{{WORKSPACE_PATH|/workspace}}` · **Host:** {{HOST_DESCRIPTION|Docker container on an EC2 host, amd64, 7 CPU / 24 GiB}}
...
- **Background jobs:** launch in a detached session — `setsid nohup ... > runs/<name>/out.log 2>&1 < /dev/null &` — write the PID to `runs/<name>/pid`, record both in `PLAN.md`, and harvest from the output file, never from memory. The `setsid` is not optional: a plain `nohup … &` child is killed when your turn ends, a detached session is not (§ Delegating work).
...
- **Experiment LLM calls:** one experiment-LLM budget, {{EXPERIMENT_LLM_BUDGET|n/a}}, spent across whichever of the providers below is provisioned. A provider is provisioned only if its key is present in your environment; if none is, experiment LLM calls are not provisioned for this run: write n/a in the ledger and do not plan around them. Whichever you use: pin exact model ids, log the model and provider returned with every response, remember that reasoning models bill hidden reasoning tokens, and on 429 back off with a bounded retry — never spin.
  - **Together AI** — key in env `TOGETHERAI_API_KEY` (also exported as `TOGETHER_API_KEY`); endpoint `https://api.together.xyz/v1` (OpenAI-compatible; the `openai` SDK with `base_url` set works). Non-obvious facts: (1) responses carry `usage` token counts but no cost — price every call yourself from Together's published per-token rates for the exact model id, log it per call, and keep `scripts/experiment_llm_costs.py` as the canonical spend check; the billing page lags. (2) There is no per-key spend limit: the cap above is yours to enforce — stop at it. (3) Together's Terms make each model's own licence govern: prefer Apache-2.0 or MIT releases for adversarial or red-teaming-class conditions. (4) The open-weight models there are reasoning models: they spend completion tokens on reasoning before the answer, so a decision-sized `max_tokens` returns `finish_reason=length` with empty content on every call. Give them a completion budget of several hundred tokens or disable reasoning for the model, and confirm one decoded decision in a smoke call before freezing a protocol on them.
  - **OpenRouter** — key in env `OPENROUTER_API_KEY`; endpoint `https://openrouter.ai/api/v1/chat/completions` (OpenAI-compatible). Non-obvious facts: (1) every response's `usage` object carries `cost` in USD — log it per call. (2) `GET https://openrouter.ai/api/v1/key` → `data.usage` (all-time USD for this key), `data.limit`, `data.limit_remaining`; a per-key credit limit, when set, is hard — calls fail with HTTP 402. `usage` is all-time, so record its launch value and subtract unless the key is fresh; it lags the calls by minutes. (3) Requests route across providers unless pinned: use versioned slugs and set `provider: {allow_fallbacks: false}` where it matters. (4) OpenRouter's Terms (§7–8) prohibit red-teaming of models — prompt injection, jailbreaking, or other adversarial action designed to compromise a model — without OpenRouter's prior written approval. Unless the operator has recorded such an approval in your environment or `inbox/`, do not route adversarial conditions through OpenRouter: use another provisioned provider, or report the gap as a broken resource (§ The operator).
...
- **Other provisioned credentials:** {{EXTRA_RESOURCES|none}} — each is a name in your environment; a name that is absent means that resource is not provisioned for this run (write n/a in the ledger and do not plan around it).
